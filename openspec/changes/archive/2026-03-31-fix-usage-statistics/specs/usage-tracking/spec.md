...
- **THEN** system records a usage log with status="success", total_tokens from response, and latency_ms calculated

#### Scenario: Failed Anthropic request

- **WHEN** an Anthropic API request fails with an error
- **THEN** system records a usage log with status="error", error_msg containing the error, and total_tokens=0
